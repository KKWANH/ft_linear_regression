...
# my utilize things
from util.output_layer import OutputLayer
...
# score calculate
myscore = numpy.sum(numpy.square(pred - numpy.average(answer))) / numpy.sum(numpy.square(answer - numpy.average(answer)))
print(f"{c.BOLD}{c.GREEN}R squared{c.RESET}      = {myscore}")

regr = linear_model.LinearRegression()
x = input_
y = answer
regr.fit(x.reshape(-1, 1), y)
plt.scatter(x, y, marker="*", label="y_real")
plt.plot(x, x * regr.coef_ + regr.intercept_)
plt.show()
print(f"{c.BOLD}{c.GREEN}regr.bias{c.RESET}      = {regr.intercept_}")
print(f"{c.BOLD}{c.GREEN}regr.weight{c.RESET}    = {regr.coef_.reshape(-1)[0]}")
print(f"{c.BOLD}{c.GREEN}regr.score{c.RESET}     = {regr.score(x.reshape(-1, 1), y)}")
